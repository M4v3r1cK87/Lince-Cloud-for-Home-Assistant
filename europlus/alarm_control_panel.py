"""Alarm Control Panel implementation for Lince Europlus."""
...
from ..const import DOMAIN  # Import dal principale
from ..utils import send_multiple_notifications, dismiss_persistent_notification
from .const import PROGRAM_BITS  # Import dalle costanti Europlus
from .parser import europlusParser  # Import locale
...
def setup_europlus_alarm_panels(system, coordinator, api, config_entry, hass):
    """
    Setup alarm control panel SPECIFICO Europlus.
    """
    entities = []
    row_id = system["id"]
    ...
    _LOGGER.debug(f"Setup Europlus alarm control panel per sistema {row_id}")
    
    # Crea il pannello di allarme Europlus
    panel = EuroplusAlarmControlPanel(
...
def _code_to_pin_list(code: Optional[str]) -> list[int]:
    """Converte codice PIN in lista di interi."""
    #if not code or not code.isdigit() or len(code) != 6:
    #    raise ValueError("Il codice deve essere una stringa di 6 cifre.")
    return [int(c) for c in code]


class EuroplusAlarmControlPanel(CoordinatorEntity, AlarmControlPanelEntity):
    """
    Alarm panel specifico per Europlus.
    ...
    Supporta anche il profilo 'vacation' con mappatura a ARMED_VACATION.
    """
    ...
    def __init__(
        ...
        self._attr_unique_id = f"lince_{row_id}_alarm"
        self._row_id = row_id
        self._api = api
    ...
    def extra_state_attributes(self):
        ...
        attrs["panel_brand"] = "lince-europlus"  # Identifica il brand
        if self._pending_state is not None:
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        try:
            ...
            if not self._initial_sync_done and current_state is not None:
                ...
                _LOGGER.info(
                    "[%s] Sincronizzazione iniziale Europlus completata. Stato: %s",
                    self._row_id,
                ...
                if current_state in [
                    ...
                    _LOGGER.info("[%s] Centrale Europlus già armata al riavvio, invio notifica stato", self._row_id)
                ...
                # Se è in TRIGGERED al riavvio, notifica importante
                    ...
                    _LOGGER.warning("[%s] Centrale Europlus in allarme al riavvio!", self._row_id)
            ...
            if previous_state != current_state and previous_state is not None:
                ...
                # Log per debug
                _LOGGER.info(
                    "[%s] Transizione stato Europlus: %s -> %s (internal_cmd: %s, pending: %s)",
                    self._row_id,
            ...
            if self._pending_state is not None:
                ...
                if self._pending_state == AlarmControlPanelState.ARMING:
                    if not tempo_out_active and progs_any:
                        _LOGGER.info(
                            "[%s] Conferma ARMING Europlus via WS. Chiudo pending.",
                            self._row_id,
                        )
                    ...
                    if progs_any is False:
                        _LOGGER.info(
                            "[%s] Conferma DISARMING Europlus via WS.",
                            self._row_id,
                        )
                        self._clear_pending(write_state=False)
                        self.hass.async_create_task(self._post_confirm_actions())
            ...
            _LOGGER.error("[%s] Errore in _handle_coordinator_update Europlus: %s", self._row_id, e)
        finally:
    ...
    def _set_error(self, msg: str):
        ...
        _LOGGER.debug(f"[Europlus Alarm Panel - {self._row_id}] {msg}")
    ...
    def _parse_ws_status(
        ...
        try:
            # Import locale del parser Europlus
            ...
            mask = 0
            if a1:
                mask |= PROGRAM_BITS.get("g1", 0)
            if a2:
                mask |= PROGRAM_BITS.get("g2", 0)
            if a3:
                mask |= PROGRAM_BITS.get("g3", 0)
            if ax:
                mask |= PROGRAM_BITS.get("gext", 0)
            ...
            _LOGGER.debug("[%s] _parse_ws_status Europlus error: %s", self._row_id, e)
            return None, False, False, False, False
    ...
    def _start_pending(
        ...
        async def _decay():
            try:
                ...
                _LOGGER.debug(
                    "[%s] Pending Europlus %s scaduto senza conferma WS",
                    self._row_id,
    ...
    async def _post_confirm_actions(self) -> None:
        ...
        try:
            ...
            if client is None:
                _LOGGER.debug(
                    "[%s] Socket Europlus non disponibile per invalidazione PIN",
                    self._row_id,
                )
                return
    ...
    async def _pin_invalidation(self, client) -> None:
        ...
        try:
            ...
            _LOGGER.debug("[%s] PIN di invalidazione Europlus inviato", self._row_id)
        except Exception as e:
            self._set_error(f"Errore invio PIN di logout Europlus {e}")
    ...
    async def _arm_with_profile(self, profile: str, code: Optional[str]) -> None:
        ...
        if configured is None:
            self._set_error(
                f"Profilo '{profile}' non configurato. Controlla le Opzioni della centrale Europlus."
            )
            return
        ...
        if not configured:
            self._set_error(
                f"Profilo '{profile}' vuoto. Controlla le Opzioni della centrale Europlus."
            )
            return
        ...
        client = self._api.get_socket_client(self._row_id)
        if client is None:
            self._set_error("Socket Europlus non disponibile.")
            return
        ...
        try:
            ...
            self._set_error(f"Errore invio PIN Europlus: {e}")
            return

        # 3) Attesa autorizzazione (10s)
        ok = await client.wait_for_authorized(self._row_id, timeout=10.0)
        if not ok:
            self._set_error("Autorizzazione Europlus non confermata (PIN errato).")
            await self._send_pin_error_notification("Inserimento")
            return
        ...
        if invalid:
            ...
            error_msg = (
                f"Attivazione Europlus negata: programmi non attivabili ({nice}). "
                "Controlla l'associazione programmi nelle Opzioni della centrale."
            )
            self._set_error(error_msg)
            await self._send_pin_error_notification("Programma")
            return
        ...
        try:
            ...
            self._clear_pending(write_state=True)
            self._set_error(f"Errore invio attivazione Europlus: {e}")
            return
    ...
    async def async_alarm_disarm(self, code: Optional[str] = None) -> None:
        ...
        if not ok:
            self._set_error("Autorizzazione Europlus non confermata (PIN errato).")
            await self._send_pin_error_notification("PIN")
            return
        ...
        try:
            ...
            _LOGGER.debug("[%s] Errore rimozione notifica TRIGGERED Europlus: %s", self._row_id, e)
        
        # Notifica DISARMED
        try:
            await self._send_disarmed_notification()
        except Exception as e:
            _LOGGER.debug("[%s] Errore invio notifica DISARMED Europlus: %s", self._row_id, e)
